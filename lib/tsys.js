...
var defaultOptions = {
  ...
  attrValueProcessor: function attrValueProcessor(a) {
    return he.encode(a, { isAttributeValue: isAttribute, useNamedReferences: true });
  } // default is a=>a
};
var options = {
  attributeNamePrefix: "@_",
    ...
    value: function makeDirectPayment(payloadJson) {
      return new Promise(function (resolve, reject) {
        ...
        axios.post(MASTER_MERCHANT_ACCESS["PaymentEndPoint"], xmls, { headers: { 'Content-Type': 'text/xml' }
          ...
          var xmlResponse = res["data"];

          if (parser.validate(xmlResponse) === true) {
          ...
          if (SaleResponse["SaleResult"]["ApprovalStatus"] == "APPROVED") {
            ...
            var _msg = SaleResponse["SaleResult"]["ApprovalStatus"];
            if (isNull(_msg) || _msg == "") {
    ...
    value: function makeRefund(payloadJson) {
      return new Promise(function (resolve, reject) {
        ...
        axios.post(MASTER_MERCHANT_ACCESS["PaymentEndPoint"], xmls, { headers: { 'Content-Type': 'text/xml' }
          ...
          var tObj = parser.getTraversalObj(xmlResponse, options);
          var jsonObj = parser.convertToJson(tObj, options);
          var SaleResponse = jsonObj["soap:Envelope"]["soap:Body"]["RefundResponse"];

          if (SaleResponse["RefundResult"]["ApprovalStatus"] == "APPROVED") {
            resolve({ "success": true, "message": jsonObj });
          } else {
            var _msg = SaleResponse["RefundResult"]["ApprovalStatus"];
            if (isNull(_msg) || _msg == "") {
